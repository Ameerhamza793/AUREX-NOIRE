...
export default async function handler(req: IncomingMessage & { body?: any }, res: ServerResponse) {
  ...
  if (!user || !pass) {
    ...
    res.end(JSON.stringify({ success: true, warning: "Email skipped: credentials not set in Vercel environment variables" }));
    return;
  }
  ...
  try {
    ...
    console.log("[send-order] SMTP verified ✅");

    // Build item cards with product image + name + qty + price
    ...
    const itemCards = items.map((item) => {
      const imageBlock = item.imageUrl
        ...
        : `<div style="width:80px;height:80px;background:#1a1a1a;border-radius:10px;border:1px solid #2a2a2a;
             display:flex;align-items:center;justify-content:center;font-size:28px;">🛍</div>`;

      const lineTotal = (parseFloat(item.price || "0") * (item.quantity || 1)).toFixed(0);
          ...
          <table style="border-collapse:collapse;width:100%;">
            <tr>
              ...
              <td style="vertical-align:middle;">
                <p style="margin:0 0 4px;color:#e5e5e5;font-size:14px;font-weight:600;line-height:1.4;">${item.name || "Product"}</p>
                <p style="margin:0;color:#888;font-size:12px;">Qty: <strong style="color:#aaa;">${item.quantity}</strong></p>
                <p style="margin:4px 0 0;color:#888;font-size:12px;">Unit: <strong style="color:#D4AF37;">${parseFloat(item.price || "0").toFixed(0)} PKR</strong></p>
              </td>
...
<div style="max-width:600px;margin:0 auto;padding:24px 16px;">
  ...
  <div style="background:linear-gradient(135deg,#111 0%,#1a1500 100%);border:1px solid #D4AF37;border-radius:16px;padding:36px 32px;text-align:center;margin-bottom:20px;">
    ...
    <p style="margin:0;color:#888;font-size:12px;letter-spacing:1px;">${now} · PKT</p>
  </div>
  ...
  <div style="background:#111;border:1px solid #1f1f1f;border-radius:14px;padding:24px;margin-bottom:16px;">
    ...
    <table style="width:100%;border-collapse:collapse;">
      ...
      <tr>
        ...
        <td style="color:#e5e5e5;font-size:13px;padding:6px 0;">
          <a href="tel:${order.customerPhone}" style="color:#D4AF37;text-decoration:none;">${order.customerPhone}</a>
        </td>
      </tr>
      <tr>
        <td style="color:#666;font-size:12px;padding:6px 0;vertical-align:top;">Email</td>
        <td style="color:#e5e5e5;font-size:13px;padding:6px 0;">
          <a href="mailto:${order.customerEmail}" style="color:#4ade80;text-decoration:none;">${order.customerEmail}</a>
        </td>
      </tr>
      <tr>
    ...
    <table style="width:100%;border-collapse:collapse;">
      <tr>
        ...
        <td style="text-align:right;vertical-align:middle;">
          <span style="color:#0a0a0a;font-size:28px;font-weight:800;letter-spacing:1px;">${parseFloat(order.totalAmount || "0").toFixed(0)}</span>
  ...
  <div style="text-align:center;padding:8px 0 24px;">
    ...
    const info = await transporter.sendMail({
      ...
      to: NOTIFY_EMAIL,
      subject: `🛒 New Order — ${order.customerName} — ${parseFloat(order.totalAmount || "0").toFixed(0)} PKR (${totalItems} item${totalItems !== 1 ? 's' : ''})`,
      html,
    });

    console.log("[send-order] ✅ Email delivered! MessageId:", info.messageId);
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify({ success: true, messageId: info.messageId }));

  } catch (err: any) {
    console.error("[send-order] ❌ EMAIL FAILED:", err?.message);
